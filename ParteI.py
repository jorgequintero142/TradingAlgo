...
def initialize(context):
    ...
    algo.attach_pipeline(make_pipeline2(), 'cantidadTweets')
    algo.schedule_function(func=record_vars2,
                           date_rule=algo.date_rules.week_start(),
                           time_rule=algo.time_rules.market_open(hours=0, minutes=30),
                           half_days=True)
...
def make_pipeline2():
    """
    Esta funcion retorna un PipeLine con informacion tomada de redes sociales
    la cual ha sido procesada en lenguaje natural para determinar poder
    cuantificar los sentimientos. En este caso, se usa informacion de twitter    
    """
    pipe = Pipeline( columns={
        'tweets': (twitter_withretweets.bull_scored_messages.latest/
...
def record_vars2(context, data):

    """
    se obtiene el la relacion entre mensajes relacionados con el alza,
    sobre el total de mensajes
    """
    context.datos_test = algo.pipeline_output('cantidadTweets')
    algo.record(por_mensajes_alza=len(context.datos_test))
...
def make_pipeline():
    ...
    tweets_bear = twitter_withretweets.bear_scored_messages.latest
    ...
    combined_factor = (
        ...
        yesterday_close.zscore()+
        yesterday_volume.zscore()+
        tweets_bear.zscore()        
    )
